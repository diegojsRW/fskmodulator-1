/*
 * rscode.js - Mar 2010 Satoshi Ueyama
 * distributed under the GNU General Public License (GPL)
 */

/* based on Reed-Solomon codec (Copyright 2002 Phil Karn, KA9Q)
...
var ReedSolomonEncoder = {
	...
	encode: function(byteBlocks, out) {
		var NN = this.NN;
		var NROOTS = this.NROOTS;
		var INDEX_OF = this.CCSDS_index_of; // array
		var ALPHA_TO = this.CCSDS_alpha_to; // array
		var GENPOLY  = this.CCSDS_poly; // array
		var MODNN = this.mod255; // func

		var feedback; // block
		var DLEN = NN - NROOTS;
		var A0 = NN;
		for(i = 0;i < NROOTS;i++) {out[i] = 0;}

		for(i = 0;i < DLEN;i++) {
			feedback = INDEX_OF[byteBlocks[i] ^ out[0]];
			if(feedback != A0) {      /* feedback term is non-zero */
				for (j = 1;j < NROOTS;j++)
					out[j] ^= ALPHA_TO[MODNN(feedback + GENPOLY[NROOTS-j])];
			}

			this.shift(out);
			if(feedback != A0)
				out[NROOTS-1] = ALPHA_TO[MODNN(feedback + GENPOLY[0])];
			else
				out[NROOTS-1] = 0;
